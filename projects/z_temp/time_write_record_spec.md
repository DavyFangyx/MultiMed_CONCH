...
覆盖率按槽位：分母=该槽对象存在的患者数，分子=算出时间的人数，单元格 `24/25`，路径 `diagnoses[].pathology_details[1]`。
...
合成病例必须覆盖：主判据命中；treatments/pathology 窄表继承父 `days_to_diagnosis`；Postoperative 且无主判据则排除；follow-up 缺天数排除；molecular/other 继承父 follow-up；comorbidity 优先于 risk_factor；输出不含 case/demographic/exposures/family_histories；旧 `time/` 被删；missing 槽位分母只计该槽存在的人。
