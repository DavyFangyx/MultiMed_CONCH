...
| `diagnoses[].treatments[]` | `days_to_treatment_start` | 无主判据时按 `timepoint_category` 置信度继承父诊断 `days_to_diagnosis` | `Recurrence`/`Progression` 或父无天数则排除 | `diagnoses_treatments_record{i}` |
| `diagnoses[].pathology_details[]` | `days_to_pathology_detail` | 无主判据时按 `timepoint_category` 置信度继承父诊断 `days_to_diagnosis` | `Recurrence`/`Progression` 或父无天数则排除 | `diagnoses_pathology_details_record{i}` |
...
treatments / pathology 无主判据时，按 `timepoint_category` 置信度继承父诊断 `days_to_diagnosis`：high=`Initial Diagnosis`/`Prior to Diagnosis`/`Preoperative`/`Pretreatment`/`Pre-treatment`/`Prior to Procurement`；medium=`Postoperative`/`First Treatment`/`Prior to Adjuvant Therapy`；low=空值或其他；drop=`Recurrence`/`Progression`。父无天数则排除。覆盖率表拆 `measured/high/medium/low`。
...
合成病例必须覆盖：主判据命中；Preoperative 高置信继承；Postoperative 中置信继承；Recurrence 不继承；空 timepoint 低置信继承；follow-up 缺天数排除；壳 follow-up 不编号且不进分母，nested 仍统计；molecular/other 继承父 follow-up；comorbidity 优先于 risk_factor；输出不含 case/demographic/exposures/family_histories；旧 `time/` 被删；missing 槽位分母只计该槽存在的人，并拆置信度。
